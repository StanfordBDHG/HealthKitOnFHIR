...
markdown_file = open('../Documentation/SUPPORT_TABLE.md', 'w')
...
def create_correlation_types_table():
    ...
    for type in correlation_types:    
        # Format code links
        coding = correlation_types[type]['codings'][0]
        code = coding['code']
        code_system = correlation_types[type]['codings'][0]['system']
        ...
        row = [type, ':white_check_mark:', linked_code]
        rows.append(row)
